...
add_shortcode('button', 'shortcode_button');
	...
	add_shortcode('divider', 'shortcode_divider');
		function shortcode_divider($atts, $html = null) {
			$html .= '<hr class="egywebschool-hr"></hr>';
...
add_shortcode('toggle', 'shortcode_toggle');
	function shortcode_toggle($atts, $content = null) {
		$atts = shortcode_atts(
			...
			return '<span><ul class="egywebschool-toggle"><li class="egywebschool_active">' . $atts['title'] . '</li><div class="div_egywebschool_toggle ' . $atts['status'] . '">' .do_shortcode($content). '</div></ul></span>';
	}
...
function jqtools_tab_group( $atts, $content ){
	...
	if( is_array( $GLOBALS['tabs'] ) ){
		...
		foreach( $GLOBALS['tabs'] as $tab ){
			if($counter == 1) {
				$tabs[] = '<li class="egywebschool-tab"><a href="#tab'.$counter.'">'.$tab['title'].'</a></li>';
			} else {
				$tabs[] = '<li class="egywebschool-tab"><a href="#tab'.$counter.'">'.$tab['title'].'</a></li>';
			}
		...
		$panes[] = '';
		$return = "".'<!-- the tabs --><div class="egywebschool-tab-container"><ul class="egywebschool-etabs">'.implode( "\n", $tabs ).'</ul>'."\n".'<!-- tab "panes" -->'.implode( "\n", $panes ).''."\n</div>";
	}
	return $return;
}
