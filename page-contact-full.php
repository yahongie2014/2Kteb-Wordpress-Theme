<?php
// Template Name: الاتصال بنا / شاشة كاملة
/**
 *
		...
		<div class="container-spacing">
			...
			<div id="contact-wrapper">
				...
				<?php if(isset($hasError)) { //If errors are found ?>
					<p class="error"><?php _e('الرجاء اعادة التحقق من البيانات التى تم ادخالها.','egywebschool'); ?></p>
				<?php } ?>

				<?php if(isset($emailSent) && $emailSent == true) { //If email is sent ?>
					<p class="success"><?php _e('تم ارسال الايميل بنجاح!','egywebschool'); ?></p>
					<p class="success2"><?php _e('شكرا لاتصالك بنا .. سوف نقوم بالرد عليك فى اقرب وقت','egywebschool'); ?></p>
				<?php } ?>
				<form method="post" action="<?php echo get_permalink(); ?>" id="contactform">
					<div class="contact-form-border-input">
					    <label for="name"><?php _e('الأسم','egywebschool'); ?>:<span class="required">*</span></label>
				    ...
				    <input type="submit" value="<?php _e('إرسال الرسالة','egywebschool'); ?>" name="submit" class="button" />
				</form>
